...
func is_image_loaded(image):
	return image in loaded_images

func mark_image_loaded(image):
	loaded_images.append(image)

func unload_image(image):
	if image in loaded_images:
		loaded_images.erase(image)

func is_image_found(image):
	return image in found_images

func mark_image_found(image):
	found_images.append(image)
	print("Image found, ", image)
	check_win_condition()

func clear_loaded_images():
	loaded_images.clear()
	found_images.clear()
...
func get_finished_cards():
	return finished_cards


# Check win condition
func check_win_condition():
	if (found_images.size() % 6) == 0:
		finished_cards.append(current_card_number)
		current_card_number += 1
		if finished_cards.size() == 3: # Assuming there are 3 cards to finish
			print("All cards finished! You win!")
